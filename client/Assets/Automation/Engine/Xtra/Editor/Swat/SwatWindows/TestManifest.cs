					...
					for(int i = 0; i < _allMethodsLast.Count; i++) {
						...
						if (AutomationMaster.UnitTestMode) {
							...
							for (int u = 0; u < ut.Length; u++) {
								...
								if (ConfigReader.GetStringList ("RESERVED_CATEGORY_NAMES").Any () && ConfigReader.GetStringList ("RESERVED_CATEGORY_NAMES").FindAll (x => ut [u].CategoryName.StartsWith (x)).Any ()) {
