...
namespace TrilleonAutomation {
   ...
   */
	...
	/// Somewhat like a Single Page Application (SPA) model web app.
	/// </summary>
	public class Swat : EditorWindow {
		...
		public static Dock DockNextTo { 
			get { 
				switch(ConfigReader.GetString("DEFAULT_DOCK_NEXUS").ToLower()) {
				case "hierarchy":
		...
		public static T ShowWindow<T>(Type type, string name, Dock dockNextTo = Dock.Default) where T : EditorWindow {
	        ...
	        */
			...
			if(dockNextTo != Dock.Default) {
				...
				dockThis = dockNextTo; //Will search for the default Dock position based on Trilleon Settings.
		...
		void Update() {
			...
			/* TODO: Background area alpha layer.
	         if(SwatPopups.Any()) {
	            
	            popping = SwatPopups.FindAll(x => x.Visible()).Any();
			...
			for(int s = 0; s < SwatWindows.Count; s++) {
				...
				//TODO: Fix.
				//if(SwatWindows[s].Shortcut.FindAll(x => Input.GetKeyDown(x)).Count == SelectedTab.Shortcut.Count) {
		...
		public void Render() {
			...
			int closeButtonWidth = 25;

			//Render tabs based on current size of window.
			float tabWidthTotal = 90 * SwatWindows.Count + closeButtonWidth;
			TabSize currentTabSize;

			if(position.width >= tabWidthTotal){

				currentTabSize = TabSize.Large;

			} else if(position.width > (tabWidthTotal/1.25)){

				currentTabSize = TabSize.Medium;

			} else {

				currentTabSize = TabSize.Small;

			}

			tabWidthTotal = position.width; //Tabs will span the entire width of the Swat window.

			EditorGUILayout.BeginHorizontal();

			//Only render the tab group if this is a primary window. Some windows in additional tools may not be included in the toolbar.
			if(SelectedTab.PriorityID >= 0) {

				for(int tb = 0; tb < SwatWindows.Count; tb++) {

					if(SwatWindows[tb].PriorityID < 0) {

						continue; //Hidden window, so no tabs.

					}
					TabDetails tabDetails = SwatWindows[tb];
					SizedTab tabToRender = SwatWindows[tb].Get(currentTabSize);

					if(SelectedTab.Window == SwatWindows[tb].Window) {

						tabButton = new GUIStyle(GUI.skin.label);
						tabButton.normal.background = TabButtonBackgroundSelectedTexture;

					} else {

						tabButton = new GUIStyle(GUI.skin.button);
						tabButton.normal.background = TabButtonBackgroundTexture;

					}

					tabButton.alignment = TextAnchor.MiddleCenter;
					tabButton.margin = new RectOffset(-2, -2, 0, 0);
					tabButton.fontSize = tabToRender.FontSize;
					tabButton.normal.textColor = tabToRender.TextColor;

					Nexus.Self.Button(tabToRender.TabText, tabDetails.Get(TabSize.Large).TabText,
						new Nexus.SwatDelegate(delegate() {                
							SelectedTab = SwatWindows[tb];
						}), tabButton, new GUILayoutOption[] {
							GUILayout.Height(25),
							GUILayout.Width(tabWidthTotal / SwatWindows.Count - (closeButtonWidth / SwatWindows.Count)),
			...
			tabButton = new GUIStyle(GUI.skin.button);
			tabButton.normal.textColor = SelectedTab.PriorityID >= 0 ? Color.red : TextGreen;
			tabButton.margin = new RectOffset(-2, -2, 0, 0);
			tabButton.fontSize = 14;
			tabButton.alignment = TextAnchor.MiddleCenter;
			tabButton.normal.background = TabButtonBackgroundTexture;
			closeButtonWidth = SelectedTab.PriorityID >= 0 ? closeButtonWidth : 50;
			...
			Nexus.Self.Button(SelectedTab.PriorityID >= 0 ? "X" : "<<<", "Show/Hide currently-selected game object in heirarchy window.", 
				...
				}), tabButton, new GUILayoutOption[] { GUILayout.Height(25), GUILayout.Width(closeButtonWidth) });
		...
		Rect TooltipRect(Vector2 mousePosition, string tooltip) {
			...
			boxWidth = messageWidth / lines;
			if(boxWidth / 2 > distanceToEdgeLeft) {

				indent = (boxWidth / 2 - distanceToEdgeLeft);

			} else if(boxWidth / 2 > distanceToEdgeRight) {

				indent = -(boxWidth / 2 - distanceToEdgeRight);

			} 

			float normalX = mousePosition.x - (boxWidth / 2); 

			return new Rect(new Vector2(normalX + indent, mousePosition.y + floatOffsetY), new Vector2(boxWidth + 10, lines * TOOLTIP_HEIGHT_MULTIPLIER));
