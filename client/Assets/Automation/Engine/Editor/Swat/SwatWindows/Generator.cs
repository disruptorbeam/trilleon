		...
		Driver driver {
			get {
				if(_objectDriver == null) {
					_objectDriver = GameObject.Find("TestMonitorHelper").GetComponent<Driver>();
				}
				if(_objectDriver == null) {
					_objectDriver = GameObject.Find("TestMonitorHelper").AddComponent<Driver>();
				}
			...
			if(!_showingGen && Application.isPlaying) {
				...
				if(_showingGen) {
					...
					if(GUILayout.Button("View Code", viewCode, new GUILayoutOption[] { GUILayout.Height(30), GUILayout.Width(110) })) {
						...
						System.Diagnostics.Process.Start(string.Format("{0}{1}RecordedTestCodeGen.html", FileBroker.BASE_NON_UNITY_PATH, ConfigReader.GetString("EDITOR_RESOURCE_FILES_DIRECTORY")));
