...
namespace TrilleonAutomation {
	...
	public class TestManifest : SwatWindow {
		...
		bool expandCategories = false;
		bool _viewModeCategory = true;
		bool _hardStop, hideBoxArea, _ignoreBuddyTests;
		...
		public override void Render() {
			...
			toolBarButtons.padding = Nexus.isPc ? new RectOffset(0, 2, 0, 0) : new RectOffset(0, 5, 0, 0);
			if(Nexus.Overseer.ignoreDependentTestsForRun) {
				toolBarButtons.normal.textColor = Color.red; 
			} else {
				toolBarButtons.normal.textColor = Nexus.TextGreen; 
			}
			Nexus.Self.Button(NODEPENDENCIES, string.Format("{0}: Toggle ignore all dependency gathering, including BuddySystem, when launching subset of tests.", Nexus.Overseer.ignoreDependentTestsForRun ? "ON": "OFF"), 
				new Nexus.SwatDelegate(delegate() {                
					Nexus.Overseer.ignoreDependentTestsForRun = !Nexus.Overseer.ignoreDependentTestsForRun;
				}), toolBarButtons, new GUILayoutOption[] { GUILayout.Width(25), GUILayout.Height(25) });
			extraToolCount++;
				...
				if(_reget) {
					...
					for(int i = 0; i < _allMethodsLast.Count; i++) {
						...
						for(int a = 0; a < aut.Length; a++) {
							...
							if(ConfigReader.GetStringList("RESERVED_CATEGORY_NAMES").Any() && ConfigReader.GetStringList("RESERVED_CATEGORY_NAMES").FindAll(x => aut[a].CategoryName.StartsWith(x)).Any()) {
				...
				Nexus.Self.Button("Multiple", "Launches selected subset of tests.", 
					new Nexus.SwatDelegate(delegate() {                
						if(!AutomationMaster.Busy) {
							...
							RunTestsAlert.Pop(AutomationMaster.AllMethodsInFramework, null, "all", "Multiple Tests");
						} else {
							if(Application.isPlaying) {
								SimpleAlert.Pop("The test runner is currently active. Stop test execution, or wait for it to complete, before launching new test runs.", null);
							}
		...
		void GetTestMethods() {
			...
			List<Type> types = AutomationMaster.GetAutomationClasses(true);
		...
		void SelectedMethodsGetData() {
			...
			List<TestRunnerFlag> tf = Attribute.GetCustomAttributes(method, typeof(TestRunnerFlag)).ToList().ConvertAll(x => (TestRunnerFlag)x);
			if(tf.Any()) {
		...
		public void LaunchTests(string name, string type) {
			...
			if(!EditorApplication.isPlaying) {
				...
				SaveEditorSettings();
				FileBroker.SaveNonUnityTextResource(FileResource.LaunchInstructions, string.Format("{0}{1}{2}", launchLine, AutomationMaster.DELIMITER, Nexus.Overseer.Master_Editor_Override.Value), true);
				...
				AssetDatabase.Refresh();
				AutomationMaster.Arbiter.ReceiveMessage(string.Format("[{{\"automation_command\": \"rt {0}\"}}]", launchLine), true);
