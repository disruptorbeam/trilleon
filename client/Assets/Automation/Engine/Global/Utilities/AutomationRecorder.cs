...
namespace TrilleonAutomation {
	...
	public class AutomationRecorder : MonoBehaviour {
		...
		public static void AutomationRelevantActionTaken(AutomationListener listener) {
			...
			if(i != null || matches.FindAll(x => x.Value == ActableTypes.Input).Any()) {
				...
				if(i != null) {
					...
					data.AsComponent = data.Components.FindIndexOf(matches.Find(x => x.Value == ActableTypes.Input).Key.Name);
         ...
         TextMeshProUGUI tmp = listener.gameObject.GetComponent<TextMeshProUGUI>();
         if(txt != <span class='value'>null</span> || tmp != <span class='value'>null</span>) {

            data.AsComponent = txt != <span class='value'>null</span> ? data.Components.FindIndexOf("Text") : data.Components.FindIndexOf("TextMeshProUGUI");
            data.ID = CurrentStepID++;
		...
		public static void GenerateFullCodeSnippet() {
			...
			snippet.AppendLine("</div>");
			FileBroker.CreateFileAtPath(string.Format("{0}{1}RecordedTestCodeGen.html", FileBroker.BASE_NON_UNITY_PATH, ConfigReader.GetString("EDITOR_RESOURCE_FILES_DIRECTORY")), snippet.ToString());
		...
		public static string GenerateInputCodeStub(RecordedGameObjectData obj, bool anyAssertions, string assert) {
			...
			stub.AppendLine(string.Format("<div class='code_line'><span class='object_declaration'>Q</span>.driver.SendKeys({0}.GetComponent&lt;<span class='variable'>InputField</span>&gt;(), {1});</div>", anyAssertions ? "currentObject" : currentObjectFind, obj.RandomStringLength > 0 ? string.Format("<span class='object_declaration'>Q</span>.help.RandomTitleString({0}, {1})", obj.RandomStringLength.ToString(), obj.RandomStringAllowSpecialCharacters ? "false" : "true") : string.Format("<span class='string_text'>\"{0}\"</span>", obj.TextValue)));
			return _lastAddedStep = stub.ToString();
